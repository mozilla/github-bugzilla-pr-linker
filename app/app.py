...
def postreceive():
    ...
    if GITHUB_WEBHOOK_SECRET == "secret" and not DEBUG:
        raise ConfigurationError("GITHUB_WEBHOOK_SECRET not set")

    # Need do a SHA check on the payload
    header_signature = request.headers.get("X-Hub-Signature")
    if header_signature is None:
        logger.warning("No X-Hub-Signature header in request")
        abort(403)
    ...
    if not hmac.compare_digest(signature, digest):
        logger.warning(
            f"Payload ({len(raw_payload)}, {type(raw_payload)}) {raw_payload[:500]!r}"
        )
